...
test("standard ENA identity-bearing exports require one explicit confirmation while aggregate exports stay direct", async () => {
  ...
  const aggregateOna = workspace.slice(workspace.indexOf('const value = await buildOnaBoundViewV3(result, currentPlan, primaryGroupName'), workspace.indexOf('>Export ONA aggregate edges'));
