...
test("persistent Plot Tools expose every controlled visual setting with accessible state", () => {
  ...
  assert.match(markup, /Scale edge weights <output>1\.4×<\/output>/);
  assert.match(markup, /Minimum edge weight <output>25%<\/output>/);
  assert.match(markup, /Scale unit circles <output>0\.8×<\/output>/);
  assert.match(markup, /Code labels[\s\S]*?checked=""/);
  assert.match(markup, /Unit labels[\s\S]*?type="checkbox"/);
  assert.match(markup, /Unit circles[\s\S]*?checked=""/);
