...
test("the browser smoke proves fallback fullscreen is one reversible keyboard-modal session", () => {
  ...
  assert.match(source, /ariaDisabled:/u);
  assert.match(source, /active:/u);
  assert.match(source, /focusableActions/u);
  assert.match(source, /currentActiveAction/u);
  ...
  assert.match(source, /JSON\.stringify\(entryState\.focusableActions\) === JSON\.stringify\(\["fullscreen"\]\)/u);
  assert.match(source, /pendingShiftTabStayedOnExit/u);
  ...
  assert.match(source, /settledState\.currentActiveAction === "fullscreen"/u);
  assert.match(source, /JSON\.stringify\(settledState\.focusableActions\) === JSON\.stringify\(\["fullscreen", "zoom-in", "zoom-out", "recenter", "copy-image"\]\)/u);
