...
| P0-04 | Approved design has no unresolved alternatives or contradictory method branches. | Pending | Open |
| P0-05 | This ledger covers every implementation, review, verification, and release-boundary criterion. | Pending | Open |
...
| P1-02 | All successful `unitColumns` are prefilled but inference remains blocked until `identityConfirmed=true`. | Pending | Open |
| P1-03 | A v1 singular repeated-entity setting migrates to one element and forces identity confirmation off. | Pending | Open |
| P1-04 | Canonical identity uses JSON field/value tuples and maps only to private opaque tokens; no delimiter-concatenated identity is used. | Pending | Open |
...
| P1-08 | `buildLongitudinalDerivation()` returns a descriptive Plot view and a private all-period comparison frame. | Pending | Open |
...
| P1-11 | Independent-period slice contains only one selected period, two disjoint groups, and one point per entity token. | Pending | Open |
| P1-12 | Paired slice inner-joins only A/B, counts earlier-only/later-only, and ignores missingness at unselected periods. | Pending | Open |
...
| P1-16 | DOM, errors, logs, and public exports contain no entity token or entity value. | Pending | Open |

## Phase 2 — Numerical engine and inference coordinator

| ID | Acceptance condition | Evidence | Status |
| --- | --- | --- | --- |
| P2-01 | Rank, 12-digit normalization, median/IQR, normal CDF, exact tails, BigInt DP, chi-square tail, and Holm are pure tested helpers. | Pending | Open |
...
| P3-11 | Result/hash/config/identity/time/order/group/period/axes changes clear inference, exports, AI consent, and interpretation. | Pending | Open |
...
| P4-03 | Longitudinal privacy flags explicitly state tokens, values, paired differences, and entity-period coordinates are absent. | Pending | Open |
...
| P4-09 | AI v2 discriminates all four designs and contains aggregate role/index/axis/count/statistic/raw-Holm/effect/method evidence only. | Pending | Open |
| P4-10 | Provider payload excludes real group names, identity values/tokens, pair differences, participant coordinates, raw rows/text, and dataset hash. | Pending | Open |
| P4-11 | Every inference cell applies minimum aggregate n; below it inference is omitted while eligible descriptive trajectory evidence remains. | Pending | Open |
...
| P4-16 | Repository/output privacy searches find no entity token, entity identity value, or individual difference in DOM/errors/exports/AI payloads. | Pending | Open |
...
This inventory was checked against the baseline repository. Existing files are marked **existing**; intended additions are marked **new**. Exact names may be narrowed during TDD, but additions cannot replace any acceptance condition above.
...
| AI prompt/route | `lib/open-ena/ai-prompt.ts`, `app/api/open-ena/interpret/route.ts` | existing when present | Versioned prompt and server compatibility |
...
| Frame tests | `tests/open-ena-longitudinal-inference-frame.test.ts` | new | Composite identity, slices, cohorts, privacy, invariance |
...
| AI tests | `tests/open-ena-ai-interpretation-payload.test.ts`, `tests/open-ena-ai-interpretation-route.test.ts`, `tests/open-ena-ai-interpretation-workspace.test.ts` | existing | V1 compatibility, v2 strictness, privacy |
