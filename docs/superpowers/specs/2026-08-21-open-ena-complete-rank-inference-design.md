...
The coordinator registers that frozen object as process-local producer authority. Strict v2 readers may parse and freeze valid imported JSON for audit and round-trip compatibility, but an imported or cloned object cannot become current producer authority. Before Stats-adjacent export, Methods, longitudinal JSON/CSV, or AI request construction, consumers validate the exact coordinator-owned object against the current comparison-group set, group-column semantics, longitudinal identity/time mapping, result/hash/hash-kind/configuration, and axes. Any stale or forged context fails closed without recomputing a statistic.

The privacy boundary for this project is exact. The internal longitudinal comparison frame and transient slice rows may contain opaque entity tokens, participant-period coordinates, and transient paired differences required for joins and numerical calculation, but never participant-level identity component values or a retained canonical identity tuple key. New local inference surfaces may retain identity column names and the aggregate study scope required by the statistical contract—selected comparison-group labels, selected period labels, axes, and provenance—but those labels are not individual participant evidence. Private frame internals never reach the inference DOM, logs, errors, public inference result, JSON/CSV, Methods inference section, or external-AI provider payload; no such new surface may expose participant-level identity component values such as participant names or IDs, canonical tuple keys, opaque entity tokens, individual pair differences, or entity-period coordinates. The external-AI provider has the stricter Section 7 boundary: real group, period, axis, and code labels and local binding fingerprints are stripped or replaced with request-local roles and indexes. Existing endpoint/general result tables and optional unit labels are separately governed legacy outputs; this project does not silently sanitize or change those surfaces. That legacy boundary does not permit private frame internals to enter a new public inference surface.
...
The result may contain composite identity column names and the selected aggregate group/period scope labels required to identify and audit the request. It never contains participant-level identity component values, retained canonical identity tuple keys, entity tokens, participant-period coordinates, individual paired differences, raw source rows, or source text. Valid but insufficient samples return typed `disabled` or `not-estimable` results. Dataset/result/config drift, identity collision, unstable group mapping, mismatched provenance, or non-finite coordinates throw a typed integrity error and fail closed.
...
`entityValuesIncluded: false` means that no participant-level identity component values or composite tuple values are included; it does not suppress identity column names or selected aggregate group/period scope labels. The geometry period CSV stays descriptive and serializes identity column names in `repeatedEntityColumnsJson`, never participant-level identity values. A separate inference CSV emits one aggregate row per axis/test/period pair, including method, statistic, raw/Holm p, privacy-safe family/member metadata, and counts. It may retain the selected aggregate group/period scope, axes, and provenance required to audit the request, but it contains no participant-level identity component value, canonical tuple key, opaque token, individual difference, or entity-period coordinate. No inferential JSON blob is repeated in geometry rows.
...
AI introduces strict request/prompt/parser v2 while the server retains the v1 parser. V2 evidence discriminates all four designs. Inferential evidence is aggregate-only: request-local group role, period index, axis role, counts, statistic, raw/Holm p, effect, request-local family/member role, and method. The application-local request binding may retain dataset hash, `analyzedAt`, selected axes, and an evidence key to enforce staleness and integrity, but the server sends only the strict v2 `evidence` projection to the external provider. The provider never receives the canonical local `familyId`, canonical member IDs, dataset hash, `analyzedAt`, filename, binding/config/result hash, reference ID, evidence key, or any other dataset-linked fingerprint. It receives only request-local role/index identifiers such as `comparison-family`, `omnibus-family`, `posthoc-family`, `axis-1`, and `period-1-period-2`; those identifiers contain no labels or stable dataset linkage. Provider content also excludes real group names, period labels, axis labels, code labels, identity column names or values, canonical identity keys, opaque tokens, participant differences, participant coordinates, raw rows, and source text. Cancellation, consent, request-size, strict validation, and rate-limit protections remain enforced at the application route.
