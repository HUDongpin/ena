...
For each target scenario, use a fresh seed state. Serialize scenarios sharing a CLI session or mutable test state; isolated scenarios may run concurrently in separate sessions:
...
Exercise the scenario through its configured test setup. Use the spec and approved requirements as the acceptance contract; the live app provides evidence of its current behavior. Resolve imprecise element references from the observed page. Change user-visible steps or expectations only when the task instructions or an accepted decision establish that the behavior is intended. Otherwise preserve the expectation and investigate a possible regression; ask only if the intended outcome remains unresolved.
...
- Close each CLI session and stop its background test when the scenario is finished, before reusing either for another scenario.

### 2.3 Generate multiple scenarios

Apply Section 2.2 to each targeted scenario, starting each from a clean seed state. Independent scenarios may run concurrently with distinct generated session names and isolated mutable state. Stop each background test run when its scenario is finished.
...
### 3.1 Identify the relevant failures

If the task supplies a failing test or current failure output, start from that evidence and target the affected test. When the failing cases are unknown, run the relevant project test command or scope:

```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/<group>/<scenario>.spec.ts
```

Use the full suite when discovering suite-wide failures or when the change scope and project gates require it. Record the relevant failing `<file>:<line>` entries. Serialize fixes that share a CLI session, files, or mutable state; independent failures may be investigated in isolated sessions.
...
- Rerun the affected test after its fix. Group related failures when they share a cause; keep concurrent work isolated and broaden validation when the change scope or project gates require it.
- When the test exposes a confirmed application bug, retain and report the failing coverage. Use `test.fixme(...)` only if existing user authorization explicitly permits deferring that regression, and link the decision or issue. Confirmation that a bug exists alone does not authorize suppressing its test.
