...
export function sha256HexUtf8(text: string) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + sum1 + choose + (SHA256_ROUND[index] ?? 0) + (words[index] ?? 0)) >>> 0;
...
export function canonicalPluginJson(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    ...
    return `{${Object.keys(record).sort().map((key) => `${JSON.stringify(key)}:${canonicalPluginJson(record[key])}`).join(",")}}`;
  }
