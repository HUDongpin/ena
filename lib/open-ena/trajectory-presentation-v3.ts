...
export function buildTrajectoryPresentationV3(result: BoundStandardResultV3, settings: TrajectoryDisplaySettingsV3) {
  ...
  const points = view.entities.flatMap((entity) => entity.steps.filter((step, index) => visible(step) && (!settings.endpointsOnly || index === entity.steps.length - 1)).map((step) => ({
    unitKey: entity.key, group: entity.group?.label ?? "All Units", horizon: step.horizon.label, horizonKey: step.horizon.key,
    ordinal: step.trajectoryOrdinal, point: step.point,
  })));
  const paths = settings.endpointsOnly ? [] : view.entities.flatMap((entity) => entity.steps.slice(1).flatMap((step, index) => {
    const previous = entity.steps[index];
    return visible(previous) && visible(step) ? [{ unitKey: entity.key, group: entity.group?.label ?? "All Units", from: previous.point, to: step.point,
  ...
  for (const entity of view.entities) for (const step of (settings.endpointsOnly ? entity.steps.slice(-1) : entity.steps)) {
  ...
  for (const entity of view.entities) for (let index = 1; index < entity.steps.length; index++) {
