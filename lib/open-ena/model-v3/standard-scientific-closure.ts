...
import type { InternalStandardRunResultV3 } from "./types";
...
export function assertStandardScientificClosureV3(plan: StandardExecutionPlanV3, runtime: InternalStandardRunResultV3, pairs: readonly (readonly [string, string | null])[], evidence: StandardScientificEvidenceV3): void {
  ...
  const align = runtime.projection.centerAlignToOrigin;
  const center = plan.reference ? plan.reference.rotationSet.centerVector : meanColumns(align ? lineWeights.filter(signal) : lineWeights);
  const inputs = plan.reference || align
  ...
  if (!plan.reference) {
    if (runtime.projection.type === "svd") {
      ...
      const centered = centerData(inputs), membership = runtime.meansBinding!;
  ...
  const geometry = plan.reference
    ? fixedNodePositions(lineWeights, plan.reference.rotationSet.nodes!.map((node) => displayed.map((axis) => Number(node[axis]))))
  ...
  const constantReference = plan.reference !== null && coordinates.every((row) => row.every((value, axis) => value === coordinates[0]?.[axis]));
