...
} from "./open-ena/analysis-workflow-eligibility-v3";
...
export interface OpenEnaWorkspaceV3Copy {
  ...
  readonly artifacts: {
    ...
    readonly unmetPrerequisites: string;
    readonly capturePredicates: Readonly<Record<AnalysisSetCapturePredicateIdV3, string>>;
...
const MODEL_SHORT_LOCALES_V3: Readonly<Record<string, readonly [string, string]>> = {
  ...
  "Unmet prerequisites": ["未滿足的前置條件", "未满足的前置条件"],
  "Rebuild and Run stay blocked until each condition below is resolved. Open an item to edit its Model control.": ["在下列每一項條件都處理完成之前，無法重新建立或執行。開啟項目即可編輯對應的模型控制項。", "在下列每一项条件都处理完成之前，无法重新构建或运行。打开项目即可编辑对应的模型控件。"],
  "Complete the backward window extent.": ["請完成向後情境範圍。", "请完成后向上下文范围。"],
  "Complete the forward window extent.": ["請完成向前情境範圍。", "请完成向前上下文范围。"],
  "Resolve the row order.": ["請處理資料列順序。", "请处理数据行顺序。"],
  "Resolve the Horizon order.": ["請處理視域順序。", "请处理视域顺序。"],
...
const en: OpenEnaCopy = {
  ...
  plot: {
    ...
    zy: "Z-Y plane",
    reset: "Reset view",
    resetNodeLayout: "Reset label positions",
  ...
  aiInterpretation: {
    ...
    consentLabel: "I reviewed this aggregate request and consent to send it to DeepSeek. Processing may occur in mainland China; zero retention and exclusion from model training are not guaranteed.",
    ...
    providerDisclosure: "Provider: this reviewed request is sent directly to DeepSeek and the model shown below.",
    ...
    retentionDisclosure: "Retention and subprocessors: this app requests a non-stored response object, but does not have a verified zero-retention or training-exclusion guarantee from DeepSeek. Review DeepSeek's current API terms and privacy disclosures before sending the aggregate preview.",
    regionDisclosure: "Processing region: DeepSeek may process data in mainland China; this app does not promise a fixed processing or storage region.",
...
const zhHant: OpenEnaCopy = {
  ...
  plot: { ...en.plot, title: "調整研究視圖", description: "這些控制只改變呈現方式，不會在未提示下重新建立模型。", showPoints: "分析單位點", showNetworks: "群組網絡", showLabels: "編碼標籤", showTrajectories: "軌跡路徑", edgeScale: "連線寬度", axisX: "X 軸", axisY: "Y 軸", axisZ: "Z 軸", camera: "相機", cameraPosition: "相機位置", default3dCamera: "預設 3D 相機", isometric: "等距", xy: "X-Y 平面", xz: "X-Z 平面", yz: "Y-Z 平面", yx: "Y-X 平面", zx: "Z-X 平面", zy: "Z-Y 平面", reset: "重設視圖", resetNodeLayout: "重設標籤位置", threeDComparisonPlot: "比較圖", threeDPrimaryPlot: "主要圖", threeDSecondaryPlot: "次要圖", threeDPlotActions: "3D 繪圖操作", zoomIn: "放大", zoomOut: "縮小", recenter: "回正", copyImage: "複製圖片", copyImageTitle: "複製繪圖圖片到剪貼簿", fullscreenEnter: "進入全螢幕", fullscreenExit: "離開全螢幕", fullscreenDialog: "全螢幕 3D 圖", actionUnavailable: "3D 視圖操作無法使用", copyingImage: "正在複製圖片", imageCopied: "圖片已複製", imageDataCopied: "圖片資料已複製", imageDownloaded: "圖片已下載", copyUnavailable: "無法複製", fullscreenOpening: "正在開啟全螢幕", fullscreenFallbackEnabled: "已啟用全螢幕備用模式", fullscreenClosed: "全螢幕已關閉", fullscreenExitFailed: "原生全螢幕無法關閉。請按 Escape 離開。", fullscreenUnavailable: "全螢幕無法使用", threeDInteractionHint: "拖曳以旋轉；滾動或使用五個繪圖操作來放大、縮小、回正、複製圖片或進入全螢幕。此幾何只作描述，不屬推論證據。", sameFittedSpace: "沿用同一個已擬合 jENA 空間；切換 2D 與 3D 不會重新執行或重新擬合分析。", threeDExportHint: "使用 3D 繪圖工具列的複製圖片按鈕，把 PNG 放到剪貼簿。SVG 與高解析度 PNG 研究圖匯出只適用於 2D 視圖。", threeDLoading: "正在載入 3D 繪圖", threeDUnavailable: "互動式 3D 暫時無法使用。已擬合結果仍保持完整；請切回 2D 或重新載入此視圖。", threeDRequiresThreeDimensions: "3D ENA 需要已完成結果具有三個不同維度；2D 結果仍可使用。" },
  ...
  aiInterpretation: {
    ...
    consentLabel: "我已審閱此彙總請求，並同意將它傳送給 DeepSeek。資料可能在中國內地處理；不保證零保留或排除模型訓練。",
    generate: "生成 AI 解讀",
    generating: "正在生成解讀…",
    cancel: "取消",
    retry: "重試",
    ...
    providerDisclosure: "供應商：此經審閱的請求會直接傳送至 DeepSeek 及下方顯示的模型。",
    dataScopeDisclosure: "傳送資料：經審閱、僅含彙總資料的 JSON 預覽；不含原始來源資料列、來源文字或參與者層級記錄。",
    retentionDisclosure: "保留期及下游處理者：本程式要求不儲存回應物件，但尚未取得 DeepSeek 零保留或排除訓練的保證。傳送彙總預覽前，請查閱 DeepSeek 現行 API 條款與私隱披露。",
    regionDisclosure: "處理地區：DeepSeek 可能在中國內地處理資料；本程式不承諾固定的處理或儲存地區。",
...
const zhHans: OpenEnaCopy = {
  ...
  aiInterpretation: {
    ...
    consentLabel: "我已审阅此汇总请求，并同意将它发送给 DeepSeek。数据可能在中国内地处理；不保证零保留或排除模型训练。",
    generate: "生成 AI 解读",
    generating: "正在生成解读…",
    cancel: "取消",
    retry: "重试",
    ...
    providerDisclosure: "供应商：此经审阅的请求会直接发送至 DeepSeek 及下方显示的模型。",
    dataScopeDisclosure: "发送数据：经审阅、仅含聚合数据的 JSON 预览；不含原始来源数据行、来源文本或参与者级记录。",
    retentionDisclosure: "保留期及下游处理者：本程序要求不存储响应对象，但尚未取得 DeepSeek 零保留或排除训练的保证。发送汇总预览前，请查阅 DeepSeek 当前 API 条款与隐私披露。",
    regionDisclosure: "处理地区：DeepSeek 可能在中国内地处理数据；本程序不承诺固定的处理或存储地区。",
