...
export interface OpenEnaCopy {
  ...
  views: { twoD: string; threeD: string; default: string; exploratory: string };
  sets: {
    title: string;
    description: string;
    capture: string;
...
const en: OpenEnaCopy = {
  ...
  views: { twoD: "2D ENA", threeD: "3D ENA", default: "Default", exploratory: "Exploratory" },
  sets: {
    title: "Analysis sets",
    ...
    emptyText: "Build an endpoint model, then capture it here. A fitted capture installs its reusable reference so a later CSV can be projected into exactly the same ENA space.",
    fitted: "Fitted",
    projected: "Projected",
  ...
  data: {
    ...
    description: "Open a CSV in this browser, or load the documented Academy sample to see the full workflow.",
    upload: "Open CSV",
    uploadHint: "CSV, up to 5 MB and 20,000 rows; model capacity also depends on units and codes",
    ...
    columns: "Columns",
    source: "Source",
    local: "Computed in this browser; the workspace has no upload endpoint.",
  },
  model: {
    ...
    sequenceNote: "CSV row order defines sequence within each conversation. Sort the file before analysis when order matters.",
    unit: "Unit",
    ...
    means: "Means (two-group contrast)",
  ...
  workspace: {
    ...
    emptyText: "Load the documented sample or open a coded CSV, map the model, and build the analysis. The comparison, primary, and secondary research frames stay visible while prerequisites are incomplete.",
    ready: "Ready",
...
const zhHant: OpenEnaCopy = {
  ...
  views: { twoD: "2D ENA", threeD: "3D ENA", default: "預設", exploratory: "探索性" },
  sets: { ...en.sets, title: "分析集", description: "將端點模型保留在瀏覽器記憶體中，並比較共享同一參照幾何的擬合或投影網絡。", capture: "擷取目前模型", captureHint: "只擷取衍生座標與等權單位網絡平均值，不保留原始來源資料列；分析單位識別碼仍會保留，必要時請先假名化。", emptyTitle: "尚未擷取分析集", emptyText: "先建立端點模型，再在這裡擷取。擬合模型會安裝其可重用參照，使之後的 CSV 可投影到完全相同的 ENA 空間。", fitted: "擬合", projected: "投影", generatedReference: "可重用擬合參照", projectionReference: "已投影至參照", sourceHash: "來源 SHA-256", primary: "主要分析集", secondary: "次要分析集", choosePrimary: "選擇主要分析集", chooseSecondary: "選擇相容的次要分析集", comparisonHint: "帶符號連線差異為共享固定幾何中的「主要減次要」。JSON 會保留分析單位識別碼；需要分享時請先假名化。", noCompatibleSecondary: "沒有可用的相容次要分析集。請在同一參照幾何中擷取或投影另一個端點模型。", remove: "移除", exportJson: "匯出比較 JSON", exportEdges: "匯出連線差異 CSV" },
  data: { ...en.data, title: "從編碼資料開始", description: "在此瀏覽器開啟 CSV，或載入已記錄的學院範例以查看完整流程。", upload: "開啟 CSV", sample: "載入教學範例", noFile: "尚未載入資料", active: "使用中的資料集", rows: "資料列", columns: "欄位", source: "來源", local: "在此瀏覽器中運算；工作區沒有資料上傳端點。" },
  model: { ...en.model, title: "定義 ENA 模型", description: "對應賦予網絡分析意義的欄位，然後執行已驗證的 jENA 流程。", sequenceNote: "CSV 資料列順序定義每段對話中的序列；若順序重要，請在分析前先排序檔案。", unit: "分析單位", conversation: "對話", group: "比較群組", identityHint: "可選一個或多個欄位；順序會定義複合識別。", noGroup: "不設比較群組（全部分析單位）", codes: "編碼", window: "窗口", movingWindow: "移動段落窗口", conversationWindow: "完整對話", back: "向後跨度（包括目前列）", forward: "向前資料列", modelType: "模型類型", endpoint: "端點（每個分析單位一個網絡）", separateTrajectory: "分離軌跡（每一步一個點）", accumulatedTrajectory: "累積軌跡（每一步為累積網絡）", trajectoryHint: "軌跡步驟依每個分析單位首次出現的對話順序排列；統計面板不會將重複步驟視為獨立分析單位。", rotation: "旋轉", svd: "SVD（資料變異）", means: "均值（兩群組對比）", center: "將零網絡分析單位置於原點", weighting: "加權", binary: "二元", run: "建立 ENA 模型", rerun: "重新建立模型", valid: "模型輸入有效" },
...
const zhHans: OpenEnaCopy = {
  ...
  views: { twoD: "2D ENA", threeD: "3D ENA", default: "默认", exploratory: "探索性" },
  sets: { ...zhHant.sets, title: "分析集", description: "将端点模型保留在浏览器内存中，并比较共享同一参考几何的拟合或投影网络。", capture: "捕获当前模型", captureHint: "只捕获派生坐标与等权单位网络均值，不保留原始来源数据行；分析单位标识符仍会保留，必要时请先假名化。", emptyTitle: "尚未捕获分析集", emptyText: "先构建端点模型，再在此捕获。拟合模型会安装其可复用参考，使之后的 CSV 可投影到完全相同的 ENA 空间。", fitted: "拟合", projected: "投影", generatedReference: "可复用拟合参考", projectionReference: "已投影至参考", sourceHash: "来源 SHA-256", primary: "主要分析集", secondary: "次要分析集", choosePrimary: "选择主要分析集", chooseSecondary: "选择兼容的次要分析集", comparisonHint: "带符号连线差异为共享固定几何中的“主要减次要”。JSON 会保留分析单位标识符；需要分享时请先假名化。", noCompatibleSecondary: "没有可用的兼容次要分析集。请在同一参考几何中捕获或投影另一个端点模型。", remove: "移除", exportJson: "导出比较 JSON", exportEdges: "导出连线差异 CSV" },
  data: { ...zhHant.data, title: "从编码数据开始", description: "在此浏览器打开 CSV，或加载已有说明的学院示例以查看完整流程。", upload: "打开 CSV", sample: "加载教学示例", noFile: "尚未加载数据", active: "当前数据集", rows: "数据行", columns: "字段", source: "来源", local: "在此浏览器中计算；工作区没有数据上传端点。" },
  model: { ...zhHant.model, title: "定义 ENA 模型", description: "映射赋予网络分析意义的字段，然后运行已验证的 jENA 流程。", sequenceNote: "CSV 数据行顺序定义每段对话中的序列；若顺序重要，请在分析前先排序文件。", unit: "分析单位", conversation: "对话", group: "比较组", identityHint: "可选一个或多个字段；顺序会定义复合标识。", noGroup: "不设比较组（全部分析单位）", codes: "编码", window: "窗口", movingWindow: "移动段落窗口", conversationWindow: "完整对话", back: "向后跨度（包括当前行）", forward: "向前数据行", modelType: "模型类型", endpoint: "端点（每个分析单位一个网络）", separateTrajectory: "分离轨迹（每一步一个点）", accumulatedTrajectory: "累积轨迹（每一步为累积网络）", trajectoryHint: "轨迹步骤按每个分析单位首次出现的对话顺序排列；统计面板不会将重复步骤视为独立分析单位。", rotation: "旋转", svd: "SVD（数据方差）", means: "均值（两组对比）", center: "将零网络分析单位置于原点", weighting: "加权", binary: "二元", run: "构建 ENA 模型", rerun: "重新构建模型", valid: "模型输入有效" },
  ...
  workspace: { ...zhHant.workspace, comparison: "比较图", groupNetworks: "组网络", emptyTitle: "构建教学示例", emptyText: "加载已有说明的示例或打开编码 CSV，映射模型并构建分析。必要条件尚未完成时，比较图、主要图和次要图框架仍会保持可见。", ready: "就绪", running: "正在使用 jENA 构建…", result: "当前模型", units: "分析单位", trajectorySteps: "轨迹步骤", codes: "编码", groups: "组", runtime: "运行环境", methodNote: "请结合来源证据以及记录的分析单位、对话、编码、窗口、加权、标准化和旋转选择来解读图形。视觉分离本身并不代表显著性或因果关系。", threeDNote: "3D ENA 探索应用程序是独立网站，不会自动接收此工作区的数据集或模型。", errorTitle: "未能构建模型", accessibleSummary: "无障碍结果摘要", groupMeans: "组平均坐标", strongestDifferences: "最强网络差异", strongestConnections: "最强网络连接", strongerGroup: "较强组", difference: "绝对差异", meanWeight: "平均权重" },
};
