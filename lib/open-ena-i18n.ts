...
const en: OpenEnaCopy = {
  ...
  aiInterpretation: {
    ...
    retentionDisclosure: "Retention and subprocessors: OpenRouter says prompt/completion logging is opt-in at its layer; Zero Data Retention (ZDR) is endpoint-specific, and downstream model-provider retention and training policies vary. Check the selected endpoint and subprocessor policies.",
...
const zhHant: OpenEnaCopy = {
  ...
  aiInterpretation: {
    ...
    retentionDisclosure: "保留期及下游處理者：OpenRouter 表示其提示／完成內容記錄在其層級為選擇性；零資料保留（ZDR）取決於端點，下游模型供應商的保留及訓練政策各異。請查閱所選端點及下游處理者政策。",
...
const zhHans: OpenEnaCopy = {
  ...
  aiInterpretation: {
    ...
    retentionDisclosure: "保留期及下游处理者：OpenRouter 表示其提示／完成内容记录在其层级为可选；零数据保留（ZDR）取决于端点，下游模型供应商的保留及训练政策各异。请查阅所选端点及下游处理者政策。",
