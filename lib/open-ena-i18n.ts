...
const en: OpenEnaCopy = {
  ...
  plot: {
    ...
    yz: "Y-Z plane",
    reset: "Reset view",
    threeDInteractionHint: "Drag to rotate; scroll or use the mode bar to zoom. The geometry is descriptive, not inferential.",
    sameFittedSpace: "Same fitted jENA space; switching between 2D and 3D does not rerun or refit the analysis.",
    threeDExportHint: "Use the 3D mode bar to download a PNG. SVG and high-resolution PNG research exports apply to the 2D view.",
...
const zhHant: OpenEnaCopy = {
  ...
  plot: { ...en.plot, title: "調整研究視圖", description: "這些控制只改變呈現方式，不會在未提示下重新建立模型。", showPoints: "分析單位點", showNetworks: "群組網絡", showLabels: "編碼標籤", showTrajectories: "軌跡路徑", edgeScale: "連線寬度", camera: "相機", isometric: "等距", reset: "重設視圖", threeDInteractionHint: "拖曳以旋轉；滾動或使用模式列縮放。此幾何只作描述，不屬推論證據。", sameFittedSpace: "沿用同一個已擬合 jENA 空間；切換 2D 與 3D 不會重新執行或重新擬合分析。", threeDExportHint: "請使用 3D 模式列下載 PNG；SVG 與高解析度 PNG 研究圖匯出只適用於 2D 視圖。", threeDUnavailable: "互動式 3D 暫時無法使用。已擬合結果仍保持完整；請切回 2D 或重新載入此視圖。" },
...
const zhHans: OpenEnaCopy = {
  ...
  plot: { ...zhHant.plot, title: "调整研究视图", description: "这些控件只改变呈现方式，不会在未提示下重新构建模型。", showPoints: "分析单位点", showNetworks: "组网络", showLabels: "编码标签", showTrajectories: "轨迹路径", edgeScale: "连线宽度", camera: "相机", isometric: "等距", reset: "重置视图", threeDInteractionHint: "拖动以旋转；滚动或使用模式栏缩放。此几何仅作描述，不属于推断证据。", sameFittedSpace: "沿用同一个已拟合 jENA 空间；切换 2D 与 3D 不会重新运行或重新拟合分析。", threeDExportHint: "请使用 3D 模式栏下载 PNG；SVG 与高分辨率 PNG 研究图导出仅适用于 2D 视图。", threeDUnavailable: "交互式 3D 暂时不可用。已拟合结果仍保持完整；请切回 2D 或重新加载此视图。" },
